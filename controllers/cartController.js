...
const populateCartItems = async (cart) => {
    const populatedItems = await Promise.all(cart.items.map(async (item) => {
        ...
        if (!product) {
            console.error("Product not found, id:", item.product);
            // Depending on your application's needs, you might decide to skip adding this item
            // to the populatedItems or handle it differently.
            return null;  // Here you return null if the product is not found.
        }
        // If the product exists, enrich the cart item with product details.
        return { 
            ...item.toObject(), 
            product: { 
                id: product._id, 
    ...
    // Filter out any null values that were added due to missing products
    const filteredItems = populatedItems.filter(item => item !== null);

    return { ...cart.toObject(), items: filteredItems };
};
